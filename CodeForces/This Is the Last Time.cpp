...
auto cmp = [](const tuple<ll, ll, ll>& a, const tuple<ll, ll, ll>& b) {
    if (get<0>(a) != get<0>(b)) return get<0>(a) > get<0>(b); // ascending
    if (get<1>(a) != get<1>(b)) return get<1>(a) > get<1>(b); // ascending
    return get<2>(a) < get<2>(b); // descending
};
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
    	ll r, l, real;
    	cin >> l >> r >> real;
    	arr[i] = make_tuple(l, r, real);
    	pq.push(arr[i]);
	}
//	
//	auto pq_copy = pq;
//	while (!pq_copy.empty()) {
//	    auto [l, r, real] = pq_copy.top();
//	    pq_copy.pop();
//	    cout << l << " " << r << " " << real << "\n";
//	}
